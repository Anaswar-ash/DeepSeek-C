...
def classify(name, n_layers, keep_mtp=False, keep_idx=False):
    if name.endswith("_scale_inv") or name.endswith(".scale"): return "consumed"
    
    if name in ("model.embed_tokens.weight", "lm_head.weight"): return "io"
    if name == "model.norm.weight": return "f32"
    ...
    if name.endswith(".weight") and "norm" in name: return "f32"
    if name.endswith("gate.bias") or name.endswith("gate_bias"): return "f32"
    if name.endswith("hc_attn_base") or name.endswith("hc_ffn_base"): return "f32"
    if name.endswith("hc_attn_scale") or name.endswith("hc_ffn_scale"): return "f32"
    if "tid2eid" in name: return "f32" # Will be saved as f32 float, engine will cast to int32
    if name.endswith("attn_sink") or name.endswith("ape"): return "f32"
    
    if ".ffn.experts." in name: return "x"
    if name.endswith(".weight") or name.endswith("_fn"): return "q"
    return "f32"
...
def main():
    ...
    # File lock removed for Windows compatibility
                ...
                try:
                    with urllib.request.urlopen(req, timeout=8) as r:
                        ...
                        while done[t] < s1 - s0:
                            ...
                            with log_lock:
                                os.lseek(fd, s0 + done[t], os.SEEK_SET)
                                os.write(fd, chunk)
                            done[t] += len(chunk)
    ...
    def _download_single(url, fn, out, part, expected):
        ...
        while True:
            ...
            try:
                ...
                nres += 1
                print(f"    [dl] {type(e).__name__} at {have/1e9:.2f} GB: resuming (#{nres})", flush=True)
                _t.sleep(min(15, 1 + nres))
        os.replace(part, out)
